package main /* Package name - main is the starting point to run a Go program */

import "fmt" /* Include the fmt package */
...
func main() {
	...
	fmt.Println("Hello, World!") /* Using the Println function from the fmt package (because Println is exported from fmt it is capitalized when we use it) */

	/* Syntax in Go */

	/* Tokens - keywords, identifiers, constants, string literals, symbols that make up a program */

	/* Don't need semicolons in Go! Detects the line separator key as a statement terminator */
